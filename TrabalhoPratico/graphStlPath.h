...
queue <TV> graphStlPath<TV,TE>::breadthFirstVisit(const TV &vContent)
{
	...
	while(!qi.empty())
	{
		...
		for(typename list < graphEdge <TV, TE> > ::iterator ite=itv-> getAdjacencias();ite++)
			if(!taken[ite->getVDestinartion()->getVKey()])
			{
...
//pesquiza em profundidade
template<class TV, class TE>
queue <TV> graphStlPath::lengthFirstVisit(const TV &vContent){
	bitset <MAX_VERTICES> taken;
	queue <TV> q;
	typename list <graphVertex<TV,TE>>iterator itv;
...
template <class TV,class TE>
void graphStlPath<TV,TE>::lengthFirstVisitRescursive(typename list < graphVertex<TV,TE> >::iterator itv, bitset <MAX_VERTICES> &taken, queue <TV> &q) const
{
	taken[itv->getVkey()]=true;
	q.push(itv->getVContent());
	for(typename list < graphEdge<TV,TE> > :: iterator itn = itv->getAdjacenciesBegin(); ite!=itv->getAdjacenciesEnd(); ite++)
		if(!take[ite->getVDestination() -> getVKey()]) 	lenghtFirstVisitRecursive(ite->getVDestination(), taken, q);
}


template <class TV, class TE>
queue <stack <TV>> graphStlPath<TV,TE>::distinctPaths(const TV &vOrigin,const &vDestination){
	bitset<MAX_VERTICES>taken;
	stack <TV> s;
	queue<stack<TV>>qr;
	typename list <graphVertex<TV,TE>>::iterator itvo,itvd;
	//verifica se existe o vector origem e destino
...
void graphStlPath<TV,TE>::distinctPathsRecursive(typename list < graphVertex <TV,TE> >::iterator itvo,typename list < graphVertex <TV,TE> >::iterator itvd, bitset <MAX_VERTICES> &taken, stack <TV> &s,  queue < stack <TV> > &qr){
	...
	s.push(itvo->getVContent());
	for (typename list <graphEdge<TV,TE>::iterator ite=itvo->getAdjencenciesBegin(); ite!=itvo->getAdjencenciesEnd(); ite++){
